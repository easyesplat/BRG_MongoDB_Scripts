...
def parse_hurricane_michael(row):
    ...
    building_specific_document = {
        ...
        "wall_fenestration_ratio": compute_average_value(row["front_wall_fenestration_ratio"], row["back_wall_fenestration_ratio"],
                                                                row["left_wall_fenestration_ratio"], row["right_wall_fenestration_ratio"]),
        "large_door_present": True if row["large_door_present"] == "yes" else False,
        "large_door_opening_type": group_to_array([row["large_door_opening_type_front"], row["large_door_opening_type_back"],
                                                    row["large_door_opening_type_left"], row["large_door_opening_type_right"]]),
...
def split_to_array(elem):
    if not isinstance(elem, str) and math.isnan(elem): return elem
